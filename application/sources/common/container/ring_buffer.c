...
uint8_t ring_buffer_put_log(ring_buffer_t *ring_buffer, void *data)
{
	uint16_t next_tail_index;
	uint16_t next_head_index;
	...
	if (data != NULL)
	{
		memcpy((logdbg_t *)(ring_buffer->buffer + ring_buffer->tail_index * ring_buffer->element_size), (logdbg_t *)data, ring_buffer->element_size);
...
void ring_buffer_getall_log(ring_buffer_t *ring_buffer)
{
    if (ring_buffer_is_empty(ring_buffer))
        return;

    uint16_t temp = ring_buffer->fill_size;
    while (temp > 0)
    {
        logdbg_t log;
        memcpy(&log,
               ring_buffer->buffer + ring_buffer->head_index * ring_buffer->element_size,
               sizeof(logdbg_t));
